 ...
 * followed by `/`, so `'#'` would never match `#workers/PyodideWorker`.
 */
export const ALIASES = [
    { find: /^#root\//, replacement: abs('./') + '/' },
    { find: /^#(components|scripts|types|workers)\b/, replacement: abs('./src') + '/$1' },
]
...
/**
 * Worker sub-build settings. The worker is a self-contained bundle: it cannot resolve a bare
 * specifier against a Blob URL, so its dependencies are bundled in — including the core package the
 * library build externalises.
 *
 * ES rather than the classic format the other packages use, because the worker loads the Pyodide
 * runtime through a dynamic `import()` of the configured index URL, which only a module worker can
 * do. Vite's inline wrapper reads this format and constructs the worker accordingly.
 */
export const WORKER = {
    format: 'es',
